...
# Flags
is $db->query('select count(*) from flags where service_id != 3 and round > 0')->array->[0], 1, 'right numbers of flags';
$db->query('select * from flags where service_id != 3 and round = 1')->hashes->map(
  sub {
    is $_->{round},  1,                'right round';
