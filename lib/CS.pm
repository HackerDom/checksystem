...
sub startup {
  ...
  $app->minion->add_task(
    scoreboard => sub {
      ...
      $pg->pubsub->json('scoreboard_info')->notify(scoreboard => $app->model('score')->scoreboard_info);
      $pg->pubsub->json('scoreboard')->notify(scoreboard => $app->model('scoreboard')->generate);
    }
  );
